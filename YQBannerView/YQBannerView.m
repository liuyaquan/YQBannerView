...
- (YQPageControl *)pageControl
{
    if (_pageControl == nil) {
        ...
        _pageControl.frame = CGRectMake(self.yq_width-pageControlWidth, self.yq_bottom-pagecontrolHeight, pageControlWidth, pagecontrolHeight);
...
- (void)setPageControlShowStyle:(YQPageControlShowStyle)pageControlShowStyle
{
    ...
    if (pageControlShowStyle == YQPageControlShowStyleNone) {
        ...
        self.pageControl.yq_x = 0;
    }else if (pageControlShowStyle == YQPageControlShowStyleCenter) {
        self.pageControl.yq_x = (self.yq_width-self.pageControl.yq_width)*0.5;
    }else if (pageControlShowStyle == YQPageControlShowStyleRight) {
        self.pageControl.yq_x = self.yq_width-self.pageControl.yq_width;
    }
}
...
- (void)setUp
{
    CGFloat imageH = self.yq_height;
    CGFloat imageW = self.yq_width;
...
{
    [self setContentOffset:CGPointMake(self.yq_width*2, 0) animated:YES];
    // 延迟0.4f 执行重置图片
    isTimeUp = YES;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    // 回到第二个imageView
    [self setContentOffset:CGPointMake(self.yq_width, 0) animated:NO];
...
- (void)reloadImage
{
    ...
    // 根据滑动计算中间的是第几个
    if (offset.x > self.yq_width) { //向右滑动
        currentImageIndex = (currentImageIndex+1) % imageCount;
    }else if(offset.x < self.yq_width){ //向左滑动
